...
export class ChatViewProvider implements vscode.WebviewViewProvider {
  ...
  private _pendingMessages: Array<{ text: string; operation: string }> = []
  private _cancelStream?: () => void

  constructor(private readonly context: vscode.ExtensionContext) {
    // Register commands that need to target the chat view
    context.subscriptions.push(
      vscode.commands.registerCommand("ysAI.openGitDiff", async (filePath: string) => {
        await this._showGitDiff(filePath)
      }),
      vscode.commands.registerCommand("ysAI.dbQueryTable", async (schema: string, table: string) => {
        this._postToWebview({
  ...
  public resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(
      (message: { type: string; payload?: unknown }) => this._handleMessage(message),
      undefined,
  ...
  public sendMessage(text: string, operation = "chat"): void {
  ...
  private async _handleMessage(msg: { type: string; payload?: unknown }): Promise<void> {
    ...
    switch (msg.type) {
      ...
      case "git-commit":
        await this._gitCommit(String(payload?.message ?? ""), payload?.files as string[] | undefined)
        break
      ...
      case "git-diff":
        await this._gitDiff(payload?.file as string | undefined, Boolean(payload?.staged))
        break
      ...
      case "db-query":
        await this._dbQuery(String(payload?.sql ?? ""), payload?.params as unknown[])
        break
  ...
  private async _handleChat(payload: Record<string, unknown>): Promise<void> {
    ...
    if (!streaming) {
      try {
        const result = await companion.apiFetch<{ output: string; files?: unknown[]; success: boolean; error?: string }>(
          "/api/ai/chat",
          { body: { query, operation, language, files } },
        )
        this._postToWebview({
          type: "chat-response",
    ...
    this._cancelStream = companion.apiStream(
      "/api/ai/stream",
      { query, operation, language, files },
      (chunk) => {
        accumulated += chunk
  ...
  private async _applyFiles(payload: Record<string, unknown>): Promise<void> {
    const files = payload.files as Array<{ path: string; content: string; action: "create" | "update" | "delete" }>
    if (!files || files.length === 0) return

    const showDiff = vscode.workspace.getConfiguration("ysAI").get<boolean>("showDiffBeforeApply", true)
    ...
    for (const file of files) {
      ...
      if (showDiff && file.action === "update") {
        ...
        await vscode.window.withProgress(
          { location: vscode.ProgressLocation.Notification, title: `YS AI: Applying ${file.path}`, cancellable: true },
          async (_, token) => {
  ...
  private async _showFileDiff(payload: Record<string, unknown>): Promise<void> {
  ...
  private async _readFile(filePath: string, requestId: string): Promise<void> {
    try {
  ...
  private async _listFiles(dir: string, requestId: string): Promise<void> {
    try {
  ...
  private async _gitStatus(requestId: string): Promise<void> {
    try {
  ...
  private async _gitCommit(message: string, files?: string[]): Promise<void> {
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: "YS AI: Committing…", cancellable: false },
      async () => {
        try {
  ...
  private async _gitPush(): Promise<void> {
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: "YS AI: Pushing…", cancellable: false },
      async () => {
        try {
  ...
  private async _gitPull(): Promise<void> {
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: "YS AI: Pulling…", cancellable: false },
      async () => {
        try {
  ...
  private async _gitDiff(file?: string, staged = false): Promise<void> {
    try {
  ...
  private async _gitCheckout(branch: string, create = false): Promise<void> {
    try {
  ...
  private async _showGitDiff(filePath: string): Promise<void> {
    const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? ""
    const absPath = path.join(workspaceRoot, filePath)
    const uri = vscode.Uri.file(absPath)
    await vscode.commands.executeCommand("git.openChange", uri).then(
      undefined,
  ...
  private async _dbQuery(sql: string, params?: unknown[]): Promise<void> {
    try {
  ...
  private async _dbTables(): Promise<void> {
    try {
  ...
  private async _dbColumns(schema: string, table: string): Promise<void> {
    try {
  ...
  private async _runTerminal(command: string): Promise<void> {
    if (!command) return
  ...
  private async _insertIntoEditor(text: string): Promise<void> {
  ...
  private _postToWebview(msg: { type: string; payload: unknown }): void {
...
function getWebviewHtml(nonce: string, mediaUri: string): string {
